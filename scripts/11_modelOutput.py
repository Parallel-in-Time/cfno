...
print(f'Shape of u0: {u0.shape}, uRef: {uRef.shape}')

if args.use_full_input:
    uInit = u0[varChoices.index(var)].T
    input = u0
else:
    uInit = u0[varChoices.index(var), iXBeg:iXEnd, iYBeg:iYEnd].T
...
uRef = uRef[varChoices.index(var), iXBeg:iXEnd, iYBeg:iYEnd].T.copy()
print(f'Shape of uInit: {uInit.T.shape}, uRef:{uRef.T.shape}')

uPred = model(input)[varChoices.index(var)].T.copy()
...
if uRef.shape != uInit.shape:
    ...
    padded_uRef[iYBeg:iYEnd, iXBeg:iXEnd] = uRef[:,:]
    padded_uPred[iYBeg:iYEnd, iXBeg:iXEnd] = uPred[:,:]
else:
    padded_uRef = uRef.copy()
    padded_uPred = uPred.copy()
...
uPred = padded_uPred[iYBeg:iYEnd, iXBeg:iXEnd]
uRef = padded_uRef[iYBeg:iYEnd, iXBeg:iXEnd]
print(f'Shape of uPred: {uPred.T.shape}, uRef: {uRef.T.shape}')
...
contourPlot(
    uPred-uRef, X, Y, title=f"Model {outType} error for {var} using sample {iSample}\nDedalus reference (dt={dataset.infos['dtInput'][()]:1.2g}s)",
    refField=None, refTitle=None,
    saveFig=f'{saveFig}_{outType}_error.jpg', closeFig=False, refScales=refScales)
print(f" -- saved {var} contour for sample {iSample}")