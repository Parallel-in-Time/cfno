...
def runSim3D(dirName, Rayleigh=1e7, resFactor=1, baseDt=1e-2/2, seed=999,
    ...
    distr = d3.Distributor(coords, dtype=dtype, mesh=mpiBlocks, comm=sComm)
    xbasis = d3.RealFourier(coords['x'], size=Nx, bounds=(0, Lx), dealias=dealias)
    ybasis = d3.RealFourier(coords['y'], size=Ny, bounds=(0, Ly), dealias=dealias)
    zbasis = d3.ChebyshevT(coords['z'], size=Nz, bounds=(0, Lz), dealias=dealias)

    # Fields
    p = distr.Field(name='p', bases=(xbasis, ybasis, zbasis))
    b = distr.Field(name='b', bases=(xbasis, ybasis, zbasis))
    u = distr.VectorField(coords, name='u', bases=(xbasis, ybasis, zbasis))

    tau_p = distr.Field(name='tau_p')
    tau_b1 = distr.Field(name='tau_b1', bases=(xbasis, ybasis))
    tau_b2 = distr.Field(name='tau_b2', bases=(xbasis, ybasis))
    tau_u1 = distr.VectorField(coords, name='tau_u1', bases=(xbasis, ybasis))
    tau_u2 = distr.VectorField(coords, name='tau_u2', bases=(xbasis, ybasis))
