    ...
    def step(self, closure):
        ...
        while n_iter < max_iter:
            ...
            if state["n_iter"] == 1:
                ...
                s = d.mul(t)
                # [DEFAULT] ys = y.dot(s)  # y*s
                ys = torch.real(y.conj().dot(s))
                if ys > 1e-10:
                    # updating memory
                    ...
                    # [DEFAULT] H_diag = ys / y.dot(y)  # (y*y)
                    H_diag = ys / torch.real(y.conj().dot(y))
                ...
                for i in range(num_old - 1, -1, -1):
                    # [DEFAULT] al[i] = old_stps[i].dot(q) * ro[i]
                ...
                for i in range(num_old):
                    # [DEFAULT] be_i = old_dirs[i].dot(r) * ro[i]
            ...
            # [DEFAULT] gtd = flat_grad.dot(d)  # g * d
            gtd = torch.real(flat_grad.conj().dot(d))
