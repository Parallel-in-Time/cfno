...
class FNOSubDomainData():
    ...
    def __init__(self,
        ...
        Args:
            ...
            nx (int): size of nx
            ny (int): size of ny
            ndomain_x (int): number of sub-domain on x
            ndomain_y (int): number of sub-domain on y
            dt (float): delta timestep 
            dim (str): FNO2D or FNO3D strategy
            tStep (int): time slice. Defaults to 1.
            T_in (int):number of input timesteps. Defaults to 1.
        ...
        self.T_in = T_in
        self.T = T
        self.subdomain_x = check_subdomain(self.nx, self.ndomain_x)
        self.subdomain_y = check_subdomain(self.ny, self.ndomain_y)
    ...
    def input_loader(self,root_path:str,samples:int):
        ...
        Returns:
           input_state (np.ndarray): input state with shape [time, samples, 4, nx, ny]
    ...
    def state_split_subdomain(self,input_state: np.ndarray):
        ...
        Args:
            input_state (np.ndarray): state with shape [time, samples, 4, nx, ny]
