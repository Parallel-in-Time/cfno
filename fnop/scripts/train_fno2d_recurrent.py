...
def main(config_file:str):
    ...
    if config.exit_duration_in_mins is not None:
        _set_signal_handler()
    ...
    loader = FNODataLoader( batch_size=data_config.batch_size,
                            ...
                            dt=data_config.dt, xStep=data_config.xStep, yStep=data_config.yStep, 
    ...
    fno_path = Path(f'{config.save_path}/rbc_fno2d_time_N{data_config.train_samples}_epoch{opt_config.epochs}_m{model_config.modes}_w{model_config.width}_bs{data_config.batch_size}_dt{data_config.dt}_tin{data_config.T_in}_run{config.run}')
    ...
    if config.verbose:
        with open(f'{fno_path}/info.txt', 'a') as file:
            ...
            file.write(f"(nTrain, nVal, nTest): {data_config.train_samples,model_config.val_samples, data_config.test_samples}\n")
